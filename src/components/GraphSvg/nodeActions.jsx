...
export const add_node = (d, t) => {
  ...
  viewCenter[0] =
    0.5 * (width / zoomData.scaleX) - zoomData.translateX / zoomData.scaleX;
  viewCenter[1] =
    0.5 * (height / zoomData.scaleY) - zoomData.translateY / zoomData.scaleY;
...
export const node_clicked = function (d, t) {
  ...
  if (previouslySelected !== d) {
    ...
    d3.select(this)
      .classed(constants.graphNodeHoverClass, true)
...
export const node_mouse_down = function (d, graphSvg) {
  if (d3.event.shiftKey) {
    graphSvg.setState({
      shiftHeld: true,
      sourceId: d3.select(this).datum().id,
    });
  } else {
...
const overClockCalculation = (d, percentage_metric, offset, endOffsetRaw) => {
  ...
  const arc = d3.arc().innerRadius(50).outerRadius(50);
...
export const wheelZoomCalculation = function (
  d,
  overclockOverride = null,
  selectedElem = null,
) {
  if (overclockOverride === null) {
    ...
    updateOverclock(
      d3.select(this).select("." + constants.overclockedTextClass),
    );
  } else {
  ...
  elementsToUpdate.each(function (d) {
    ...
    if (allowedInRemaining.length > 0) {
      element
        ...
        .attr("height", 25)
        .attr("width", 25);
      const fetchRemainingIn = allowedInRemaining.map(
        (item) => itemAccessor.filter((findItem) => item === findItem.id)[0],
      );
      ...
      fetchRemainingIn.forEach((remaining) => {
        ...
        element
          ...
          .on("click", function (d) {
            const findSuitableSource = t.graphData.nodes.filter(
              (node) =>
                node.id !== d.id &&
                node.open_out_slot > 0 &&
                node.allowedOut.includes(remaining.id),
            );
            const thisNode = t.graphData.nodes.filter(
              (node) => node.id === d.id,
            );
    ...
    if (allowedOutRemaining.length > 0) {
      element
        ...
        .attr("height", 25)
        .attr("width", 25);
      const fetchRemainingOut = allowedOutRemaining.map(
        (item) => itemAccessor.filter((findItem) => item === findItem.id)[0],
      );
      ...
      fetchRemainingOut.forEach((remaining) => {
        ...
        element
          ...
          .on("click", function (d) {
            const findSuitableTarget = t.graphData.nodes.filter(
              (node) =>
                node.id !== d.id &&
                node.open_in_slots > 0 &&
...
export const forceUpdateComponentLabel = function () {
  updateComponents.call(
    this,
    d3.selectAll("." + constants.nodeRequirementsIconClass),
  );
};

export const insertComponents = function (parentElement) {
  const el1 = parentElement
    .append("g")
    .classed(constants.nodeRequirementsIconClass, true);
  ...
  const el3 = parentElement
    .append("g")
    .classed(constants.nodeLimitingThroughputClass, true);
