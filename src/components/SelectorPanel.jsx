...
import {
  updateNodeTierExternal,
  wheelZoomCalculation,
} from "./GraphSvg/nodeActions";
...
class SelectorPanel extends React.Component {
  constructor(props) {
    ...
    const items = JSON.parse(
      JSON.stringify(props.items.filter((item) => !item.hidden)),
    );
    ...
    this.items = items;
    this.state = {
      quantity: props.selected.quantityPerInfinite || 0,
      time: props.selected.timePerInfinite || 1,
      dummy: false,
      sliderValue: props.selected.overclock,
      selectedSource: props.selected.allowedOut || [],
    };
  }
  ...
  update = () => {
    if (this.props.selected.machine) {
      const el = d3.select(
        "#" + "node-level-accessor-" + this.props.selected.id,
      );
      updateNodeTierExternal(el);

      this.props.selected.overclock = this.state.sliderValue;
      const el2 = d3.select(
        "#" + "node-overclock-accessor-" + this.props.selected.id,
      );
      wheelZoomCalculation.call(
        this,
        this.props.selected,
      ...
      const el = d3.select(
        "#" +
          "path-parent" +
          this.props.selected.source.id +
          "-" +
          this.props.selected.target.id,
      );
      updateNodeTierExternal(el);
    }
  };
  ...
  infiniteSourceEligible = () => {
    return (
      (this.props.selected.machine &&
        this.props.selected.machine.name === "Container" &&
        this.props.selected.open_in_slots === 1) ||
      (this.props.selected.infiniteSource &&
        this.props.selected.open_in_slots === 0)
    );
  };
  ...
  handleChange = (event) => {
    ...
    if (this.props.selected.open_out_slots === 0) {
      usedSet = Array.from(
        new Set([
          ...event.target.value,
          this.props.selected.allowedInClobbered,
        ]),
      );
    }
  ...
  shouldShowOverclock = () => {
    return (
      ...
      this.props.selected.machine.name !== "Container" &&
      this.props.selected.machine.name !== "Logistic"
    );
  };
    ...
    return (
      <React.Fragment>
        ...
        <Hidden mdUp implementation="css">
          <Paper className={classes.paperMobile}>{this.renderContent()}</Paper>
        </Hidden>
      </React.Fragment>
    );
  }
