...
class SidebarPanel extends React.Component {
  ...
  handleChangeMultiple = (event) => {
    ...
    for (let i = 0, l = options.length; i < l; i += 1) {
      if (
        ...
        if (
          this.state.selectedDrives.indexOf(
            options[i].getAttribute("value"),
          ) === -1
        ) {
  ...
  handleCheckboxChange = () => {
    this.props.parentState.graphSvg.setState(
      { snapToGrid: !this.props.parentState.graphSvg.state.snapToGrid },
      () => {
        this.setState({
          checked:
            this.props.parentState.graphSvg &&
  ...
  showHideLines = () => {
    this.setState({ showAllLines: !this.state.showAllLines }, () => {
      if (this.state.showAllLines) {
        d3.selectAll(".link-has-no-problems").classed(
          "hiddenGraphElement",
          false,
        );
      } else {
    ...
    return (
      <Paper className={classes.paper}>
        ...
        <FormControlLabel
          ...
          className={classes.checkboxLabel}
        />
        {useExperimentalFeature("nodePhysics") ? (
          <Button
