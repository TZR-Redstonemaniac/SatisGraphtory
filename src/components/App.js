...
const theme = createTheme({
  ...
  palette: {
    mode: "light",  // Updated from type: "light"
    primary: { main: "#FF9100" },
    secondary: { main: "#FF3D00", contrastText: "#FAFAFA" },
  },
  themeName: themeName,
...
class App extends Component {
  constructor(props) {
    super(props);
    let n =
      window.localStorage.getItem("nightMode") != null
  ...
  handleNightToggle = () => {
    let newPaletteType =
      window.localStorage.getItem("nightMode") === "light" ? "dark" : "light";
    this.setState({
  ...
  generateRecursiveStructureFireBase(startingTable) {
    ...
    return Promise.resolve(starting).then(async (results) => {
      if (results.length > 0) {
        ...
        const refKeysToProcess = Object.keys(results[0]).filter((str) =>
          str.endsWith("_id"),
        );
        ...
        while (refKeysToProcess.length > 0) {
          ...
          if (!globalStructure[tableName]) {
            globalStructure[tableName] =
              await this.getRefkeyTableFireBase(tableName);
            Object.keys(globalStructure[tableName])
              .filter((str) => str.endsWith("_id"))
              .forEach((name) => {
          ...
          rows.forEach((row) => {
            Object.keys(row)
              ...
              .forEach((rowKey) => {
                ...
                delete row[rowKey];
                const possibleData = associatedData.filter(
                  (elem) => elem.id === refId,
                );
              ...
              .forEach((rowKey) => {
                ...
                const replaceTable = (id, id_name, object) => {
                  if (!id_name.endsWith("_id")) {
                    if (
                      typeof object[id_name] === "string" &&
                      object[id_name].startsWith("http")
                    ) {
                      const img = new Image();
  ...
  componentDidMount() {
    ...
    loadHash().then((data) => {
      console.log("Core Graph Data:", data); // Log data to ensure it's valid
      this.setState({ coreGraphData: data }, () => {
        this.setState({ fbdb: globalData, loaded: true }, () => {
          const generate = (name, cb = () => {}) => {
            return () => {
              return this.generateRecursiveStructureFireBase(name)
                .then((data) => {
                  console.debug("Data loaded for", name, data); // Log each loaded item
                  this.setState({ [name]: data }, () => {
                    console.debug("Loaded", name);
                    cb();
                  });
                })
                .catch((err) => console.error("Error loading", name, err)); // Catch any errors
            };
          };

          let wrappedFunction = () => this.setState({ isLoaded: true });
          const list = [
            "spring",
            "recipe",
            "path_type",
  ...
  generateUnlocksList() {
    const dataList = [];
    this.state.player_unlock &&
      this.state.player_unlock.player_unlock.forEach((player_unlock) => {
        const item = this.state.recipe.recipe.filter(
          (elem) =>
            elem.player_unlock && elem.player_unlock.id === player_unlock.id,
        )[0];
        if (item) {
  ...
  drawerContents = () => {
    ...
    return (
      <React.Fragment>
        ...
        <SidebarPanel
          parentState={this}
          playerUnlock={this.state.player_unlock}
        />
        ...
        <List>
          ...
          >
            ...
            <Typography variant="body1">
              Not officially affiliated with Satisfactory, Coffee Stain Studios
              AB, or THQ Nordic AB.
            </Typography>
            <Typography variant="body1">
          ...
          >
            <ul>
              <Typography variant="body1">
                <li>
                  Resource nodes do not have purities displayed on the graph.
                </li>
              </Typography>
              <Typography variant="body1">
        ...
        <ThemeProvider theme={this.state.theme}>
          <AppBar position="fixed" className={classes.appBar}>
            <Toolbar>
              ...
              <NightToggle
                t={t}
                onNightToggle={this.handleNightToggle}
                night={this.state.night}
              />
              {useExperimentalFeature("opt") ? (
                <Button color="inherit" onClick={() => t.graphSvg.optimize()}>
                  <OfflineBoltIcon />
                  <Hidden smDown implementation="css">
          ...
          <FabPopup>
            ...
            <Typography variant="body1">
              This is a factory planner/optimizer/analyzer tool for factories
              old and new, meant to accompany the game Satisfactory by Coffee
              Stain Studios.{" "}
            </Typography>
            <br />
            <Typography variant="body1">
              We are looking for more developers! If you know React, reach out
              to us on our{" "}
              <Link
            ...
            <ul>
              <Typography variant="body1">
                <li>
                  Use the <b>left menu</b> to <b>add</b> machines to the diagram
                </li>
              </Typography>
              ...
              <Typography variant="body1">
                <li>
                  Hold down <b>SHIFT</b> and <b>drag</b> from node to node to
                  create belts
                </li>
              </Typography>
              <Typography variant="body1">
                <li>
                  Use <b>MOUSE SCROLL</b> to control overclock (black text in
                  the white circle)
                </li>
              </Typography>
            </ul>
            ...
            <Typography variant="body1">
              Revisit these instructions anytime by clicking on the <b>?</b> in
              the bottom right.
            </Typography>
          </FabPopup>
          ...
          {/*   : null}*/}
          {this.state.selectedNode || this.state.selectedPath ? (
            <SelectorPanel
              items={this.state.item.item}
              label="Options"
              graphSvg={this.graphSvg}
              overclock={
                this.state.selectedNode ? this.state.selectedNode.overclock : -1
              }
              selected={this.state.selectedNode || this.state.selectedPath}
            />
          ) : null}
          ...
          <div
            id="svgParent"
            className={`${classes.content} ${this.state.night === "dark" ? "dark" : ""}`}
          >
            {this.state.loaded ? (
              <GraphSvg
                parentAccessor={this}
